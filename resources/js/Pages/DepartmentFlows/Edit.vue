<template>
  <Head :title="`Edit Flow Step #${form.step_order}`" />
  ...
  <AuthenticatedLayout>
    ...
    <div class="card">
      <div class="card-body">
        <h5 class="card-title mb-4">Edit Flow Step</h5>
...
const form = useForm({
  final_department_id: props.flow?.final_department_id || "",
  step_department_id: props.flow?.step_department_id || "",
  step_order: props.flow?.step_order || 1,
  is_required: props.flow?.is_required ?? true,
});

const submit = () => {
  form.put(route("department-flows.update", props.flow.id));
};
</script>
